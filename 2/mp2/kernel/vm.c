...
// #define PG_REPLACEMENT_USE_FIFO
// #define PG_REPLACEMENT_USE_LRU
...
#ifdef PG_REPLACEMENT_USE_LRU
// TODO
if(!(va == 0x0 || va == 0x1000 || va == 0x2000)) lru_push(&lru, (uint64) pte);
#elif defined(PG_REPLACEMENT_USE_FIFO)
// TODO
if(!(va == 0x0 || va == 0x1000 || va == 0x2000)) q_push(&q, (uint64) pte);
#endif
  return pte;
}
...
int madvise(uint64 base, uint64 len, int advice) {
  ...
  if (advice == MADV_NORMAL) {
    ...
    for (uint64 va = begin; va <= last; va += PGSIZE) {
      ...
      #ifdef PG_REPLACEMENT_USE_LRU
      // TODO
      if((*pte & PTE_P) == 0) lru_pop(&lru, lru_find(&lru, (uint64) pte));
      #elif defined(PG_REPLACEMENT_USE_FIFO)
      // TODO
      if((*pte & PTE_P) == 0) q_pop_idx(&q, q_find(&q, (uint64) pte));
      #endif
...
void ptTravsal(pagetable_t pagetable, int level, int lv2_last, int lv1_last, uint64 pva)
{
  if(level == 1){
    for(int i = 511; i >= 0; i--)
    {
  ...
  for(int i = 0; i < 512; i++)
  {
    if(i == lv2_last && level == 0) lv2_last = -1;
    if(i == lv1_last && level == 1) lv1_last = -1;
    pte_t pte = pagetable[i];
    uint64 pa = PTE2PA(pte);
    uint64 blockno = PTE2BLOCKNO(pte);
    uint64 va = pva + ((uint64)i << (12 + (2-level)*9));
    if((pte & PTE_V) || (pte & PTE_S))
    {
      // printf("level %d, lv2_last %d, lv1_last %d\n", level, lv2_last, lv1_last);
      for (int j = 0; j <= level; j++){
        if (j == level) printf("+-- ");
        else{
          if (j == 0){
            if (lv2_last == -1) printf("    ");
            else printf("|   ");
          }
          else if (j == 1){
            if (lv1_last == -1) printf("    ");
            else printf("|   ");
          }
        }
      }
      printf("%d: pte=%p va=%p", i, pagetable + i, va);
      ((pte & PTE_V))? printf(" pa=%p", pa) : printf(" blockno=%p", blockno);
    ...
    if(((pte & PTE_V) || (pte & PTE_S)) && level < 2){ // level 1
      ptTravsal((pagetable_t)pa, level + 1, lv2_last, lv1_last, va);
    }
  }
}
...
void vmprint(pagetable_t pagetable) {
  ...
  ptTravsal(pagetable, 0, lv2_last, -1, 0);
}
