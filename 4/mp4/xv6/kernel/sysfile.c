...
uint64
findsymlinks(struct inode *dp, char *target, char *buf, char *path){
  int off;
  struct dirent de;
  struct inode *ip;
  ...
  for(off=2*sizeof(de); off<dp->size; off+=sizeof(de)){
    ...
    if(ip->type == T_SYMLINK) {
      ...
      readi(ip, 0, (uint64)tmp, 0, MAXPATH);
      
      if(strncmp(tmp, target, strlen(target)) == 0) {
        // Found a symlink to target, append its path to buf
        strcat(buf, newpath);
        strcat(buf, " ");
      }
      ...
      iunlock(ip); //!
      findsymlinks(ip, target, buf, newpath);
      ilock(ip);
    }
    iunlockput(ip);
  }
...
sys_revreadlink(void) 
{
  ...
  findsymlinks(root, target, userbuf, "");
  int len = strlen(userbuf);
  if(userbuf[len-1] == ' ')
    userbuf[len-1] = 0; // Remove trailing space
